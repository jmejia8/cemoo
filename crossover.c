...
void realcross(float *new_pop_ptr,float *parent, int popsize, int nvar, float pcross, float lim_r[nvar][2], float di)
{
	int i,j,y,n;
	...
	for(i = 0; i < popsize/2; i++)
	{
		...
		if(rnd <= pcross)
		{
			...
			{ 
				...
				par1 = parent[y*nvar + j];
				par2 = parent[(y+1)*nvar + j]; 
				...
				new_pop_ptr[n*nvar + j] = chld1;
				new_pop_ptr[(n+1)*nvar + j] = chld2;
			}
		}
		else
		{
